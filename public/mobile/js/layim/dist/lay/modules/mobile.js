...
layui.define(["laytpl", "upload-mobile", "layer-mobile", "zepto"], function(i) {
	...
	var p = function(i) {
		...
		f = function(i, e, a) {
			return ['<div class="layim-panel' + (e ? " layui-m-anim-left" : "") + '">', '<div class="layim-title" style="background-color: {{d.base.chatTitleColor}};">', "<p>", a ? '<i class="layui-icon layim-chat-back" layim-event="back">&#xe603;</i>' : "", '{{ d.title || d.base.title }}<span class="layim-chat-status"></span><a class="navRightBtn" style="float:right;"><i class="fa fa-user-o"></i></a>', "{{# if(d.data){ }}", '{{# if(d.data.type === "group"){ }}', '<i class="layui-icon layim-chat-detail" layim-event="detail">&#xe613;</i>', "{{# } }}", "{{# } }}", "</p>", "</div>", '<div class="layui-unselect layim-content">', i, "</div>", "</div>"].join("")
		},
	...
	var b, x, w = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">', '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>', '{{ d.username||"佚名" }}', "</cite></div>", '<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp;") }}</div>', "</li>"].join(""),
		...
		O = function(i) {
			...
			if(C[i.type])
				if("friend" === i.type) layui.each(C.friend, function(t, l) {
		...
		U = {
			...
			faceHide: function() {
				...
				//june修改
				var inputCon=jQuery(".layim-chat-send input").val();
				if(inputCon!=""){
					jQuery(".layim-chat-send button").show();
				}
			},
			image: function(i) {
			...
			about: function() {
				n.open({
					content: '<p style="padding-bottom: 5px;">LayIM属于付费产品，欢迎通过官网获得授权，促进良性发展！</p><p>当前版本：layim mobile v' + e + '</p><p>版权所有：<a href="http://layim.layui.com" target="_blank">layim.layui.com</a></p>',

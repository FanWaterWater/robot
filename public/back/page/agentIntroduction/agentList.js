...
layui.use(['form', 'layer', 'laydate', 'table', 'laytpl', 'ajaxUrl'], function () {
    ...
    var ajaxUrl = ajaxArr.agentIntroductionsList;
    //列表
    ...
    function del(data){
        layer.confirm('确定删除该数据？',{icon:3, title:'提示信息'},function(index){
            var ajaxUrl = ajaxArr.agentIntroductionsDelete.url;
            var ajaxType = ajaxArr.agentIntroductionsDelete.method;
    ...
    $(".delAll_btn").click(function(){
        var checkStatus = table.checkStatus('listTable'),
            ...
            ids = [];
            ajaxUrl = ajaxArr.agentIntroductionsDelAll.url;
            ajaxType = ajaxArr.agentIntroductionsDelAll.method;
        if(data.length > 0) {
            for (var i in data) {
                ids.push(data[i].id);
            }

...
layui.use(['form', 'layer', 'laydate', 'table', 'laytpl', 'ajaxUrl'], function () {
    ...
    var ajaxUrl = ajaxArr.videosList;
    //列表
    var tableIns = table.render({
        ...
        cols: [[
            ...
            {field: 'url', title: '内容', minWidth: 100, align: "center"},
    ...
    function add() {
        var index = layui.layer.open({
            title: "添加",
            type: 2,
            content: "videoEdit.html?mode=add",
    ...
    function edit(data) {
        var index = layui.layer.open({
            title: "编辑",
            type: 2,
            content: "videoEdit.html?mode=edit",
            success: function (layero, index) {
                var body = layui.layer.getChildFrame('body', index);
                if (data) {
                    ...
                    body.find("#videoBlock").show();
                    body.find("#video").attr('src', data.url);
                    form.render();
                }
            }
        });
    ...
    function del(data){
        layer.confirm('确定删除该数据？',{icon:3, title:'提示信息'},function(index){
            var ajaxUrl = ajaxArr.videosDelete.url;
            var ajaxType = ajaxArr.videosDelete.method;
    ...
    $(".delAll_btn").click(function(){
        var checkStatus = table.checkStatus('listTable'),
            ...
            ids = [];
            ajaxUrl = ajaxArr.videosDelAll.url;
            ajaxType = ajaxArr.videosDelAll.method;
        if(data.length > 0) {
            for (var i in data) {
                ids.push(data[i].id);
            }

...
layui.use(['form', 'layer', 'laydate', 'table', 'laytpl', 'ajaxUrl'], function () {
    ...
    var tableIns = table.render({
        ...
        cols: [[
            ...
            {field: 'wechat', title: '微信号', minWidth: 150, align: "center"},
    ...
    function add() {
        var index = layui.layer.open({
            title: "添加",
            type: 2,
            content: "wxfaqEdit.html?mode=add",
    ...
    function edit(data) {
        var index = layui.layer.open({
            title: "编辑",
            type: 2,
            content: "wxfaqEdit.html?mode=edit",
            success: function (layero, index) {
                var body = layui.layer.getChildFrame('body', index);
                if (data) {
   ...
   $(".delAll_btn").click(function(){
        var checkStatus = table.checkStatus('listTable'),
            ...
            ids = [];
            ajaxUrl = ajaxArr.wxfaqDelAll.url;
            ajaxType = ajaxArr.wxfaqDelAll.method;
        if(data.length > 0) {
            for (var i in data) {
                ids.push(data[i].id);
            }
    ...
    table.on('tool(list)', function (obj) {
        ...
        if (layEvent === 'edit') { //编辑
            ...
            layer.confirm('确定删除此记录？',{icon:3, title:'提示信息'},function(index){
                var ajaxUrl = ajaxArr.wxfaqDelete.url;
                var ajaxType = ajaxArr.wxfaqDelete.method;
